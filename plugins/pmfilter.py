...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔍 ᴊᴏɪɴ ᴀɴᴅ  𝙍𝙀𝙌𝙐𝙀𝙎𝙏 𝙃𝘼𝙍𝙀 🔎", url=f"https://t.me/Movie_Searching_Pro")]]))
...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         ...
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝  𝙍𝙀𝙌𝙐𝙀𝙎𝙏 𝙃𝘼𝙍𝙀 📝", url=f"https://t.me/MKV_FILES_ROHESH")]])
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if settings is not None:
            buttons = [
                [
                    ...
                    InlineKeyboardButton('ʙᴜᴛᴛᴏɴ' if settings["button"] else 'Link',
                                         callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}')
                ],
                [
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("📝  𝙍𝙀𝙌𝙐𝙀𝙎𝙏 𝙃𝘼𝙍𝙀 📝", url="https://t.me/MKV_FILES_ROHESH")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        try:
            ...
            if await db.has_premium_access(user_id):                               
                buttons = [[
                    ...
                    InlineKeyboardButton('🔰 𝙅𝙊𝙄𝙉 𝙐𝙋𝘿𝘼𝙏𝙀 𝘾𝙃𝘼𝙉𝙉𝙀𝙇 🔰', url='https://t.me/Filmy_Rohesh')
                ]]
            else:
    ...
    # don't change anything without contacting me @creatorrio
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🔍 💠 𝙂𝙍𝙊𝙐𝙋 1♕ 💠', url='https://t.me/Movie_Searching_Pro'),
            InlineKeyboardButton('💠 𝙂𝙍𝙊𝙐𝙋 2♕ 💠 🔎', url='https://t.me/+JKeloMFcoYpjMGM1')
        ],[
            InlineKeyboardButton('📝  𝙍𝙀𝙌𝙐𝙀𝙎𝙏 𝙃𝘼𝙍𝙀 📝', url='https://t.me/MKV_FILES_ROHESH')
        ],[
            InlineKeyboardButton('⇇ ʙᴀᴄᴋ', callback_data='start'),
            InlineKeyboardButton('👮 𝙎𝙐𝙋𝙋𝙊𝙍𝙏 👮', url='https://t.me/Rohesh_Bots')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f'Seaching for {search} 🔎', url=f"https://t.me/MKV_FILES_ROHESH")]]) 
            )
            search = search.lower()
            find = search.split(" ")
            search = ""
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        if not g_s:
            ...
            button = [[
                       InlineKeyboardButton("📝  𝙍𝙀𝙌𝙐𝙀𝙎𝙏 𝙃𝘼𝙍𝙀 📝, url=f"https://t.me/MKV_FILES_ROHESH")
            ]]
            if NO_RESULTS_MSG:
